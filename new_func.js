...
function startScreen2(deptData) {
    ...
    document.getElementById('btn-submit-mission-1-2').onclick = async () => {
        ...
        if (isCorrect) {
            ...
            if (currentRole === '부장') {
                document.getElementById('btn-submit-mission-1-2').textContent = '최종 확인 완료 V';
                ...
                document.getElementById('btn-submit-mission-1-2').textContent = '단서 전달 완료 V';
        ...
        onSnapshot(collection(db, `departments/${currentDeptId}/roles`), (snapshot) => {
            ...
            requiredRoles.forEach(role => {
                ...
                if (isConfirmed) {
                    ...
                    statusEl.querySelector('.status-icon').textContent = '[완료]';
                    statusEl.style.background = 'rgba(0,100,0,0.5)';
                } else {
                    statusEl.classList.remove('done');
                    statusEl.querySelector('.status-icon').textContent = '[대기]';
